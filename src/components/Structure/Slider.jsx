...
function GenerationDuSlider() {
	...
	const imageDuSlider = IdDuLogement.pictures.map((image, index) => (
		<img
			...
			className={`slider__logement__img ${
	...
	const nextSlide = () => {
		setImageIndex((nextIndex) =>
			nextIndex === IdDuLogement.pictures.length - 1 ? 0 : nextIndex + 1
		);
	};
	if (totalSlides === 1) {
		return (
			<div id="slider__logement">
				<img
					src={IdDuLogement.pictures}
					alt="Vue du logement"
					className="slider__logement__img active"
				/>
			</div>
		); // Ne rend rien si vous avez une seule photo
	}
	return (
		<div id="slider__logement">
